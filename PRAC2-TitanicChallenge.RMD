...
A l'hora de resoldre la pràctica poso entre parèntesis al costat de cada apartat el número al que correspon de l'enunciat de la pràctica per a una correcció més fàcil
...
*****
## Completesa, adaptació i selecció de les dades a utilitzar (3.1 i 4.1)
*****
...
*****
## Comprovació de la normalitat i homogeneitat de la variança (4.2)
*****
...
Encara havent comprovat la normalitat utilitzant la llibreria nortest, faré una representació més visual de la distribució de les dades per veure com de llunyana és la seva distribució d'una distribució normal:
```{r message= FALSE, warning=FALSE}
# Extrec els noms de les columnes, els usare de titul epr als plots
titles <- colnames(dataset)

# Per cada columna del dataset menys la primera (correspon a l'atribut supervivència)
for (i in 2:ncol(dataset)) {
  # Genero el gràfic de quantils teòrics per a cada atribut
  qqnorm(dataset[,i], main = titles[i])
  # Afegeixo una linea de referència
...
```
D'aquesta manera en observar la normalitat dels diferents atributs de manera visual s'observa com:
1. Pclass, Sex i Embarked estàn molt lluny de seguir una distribució normal en ser atributs que només tenen 2 o 3 valors possibles.
2. SibSp, Parch i Priority estàn lluny de seguir una distribució normal ja que encara tenint més rang de valors aquests estan mal distribuits pel que fa a freqüència d'aparició.
3. Fare també està lluny de seguir una distribució normal perquè la gran majoria de valors de Fare són menors a 15$ encara tenint un rang de valors que va de 0 a més de 250.
4. Age és l'atribut que més s'apropa a una distribució normal en haver pasatgers de totes les edats però, encara així, no segueix una distribució normal ja que hi ha molta gent d'entre 20 i 38 però hi ha més o menys al mateixa quantitat de gent entre 0 i 20 que entre 38 i 80 anys.
 

*****
## Diagrames de freqüència, taules i gràfiques (5 i 4.3.3)
*****
...
```
**4.3.3 - tests estadistics de les taules de correlacions** 
...
*****
# Generació i aplicació de models (4.3.4 + 5 + 6 - generació d'un model estadistic, representació d'aquest i resolució del problema)
*****
...
*****
# Resolució del problema i conclusions (6)
*****
